...
export class Easy {
    ...
    static CreateJSONSprite(jsonGroup, name, _alpha) {
        ...
        var _x = jsonGroup[name].left;
        var _y = jsonGroup[name].top;
    ...
    //弃用
    static CreateJSONSpriteOLD(jsonGroup, name, _alpha) {
        var url = jsonGroup[name].url;
    ...
    static CreateInput(placeholder = '请输入...', maxlength = 5, style = {}, _class = 'myinput') {
        ...
        if (style.left) {
            style.left = MyData.stage.x + MyData.scale * style.left + 'px';
        }
        if (style.top) {
            style.top = MyData.stage.y + MyData.scale * style.top + 'px';
        }
        if (style.width) {
            style.width = MyData.scale * style.width + 'px';
        }
        if (style.height) {
