...
export default class Preload extends PIXI.Container {
    constructor() {
        ...
        this.files = require.context('./static/images', true, /\.png$|\.jpg$|\.webp$/);
        this.imgModules = {};
        ...
        Object.keys(this.imgModules).forEach(e => {
            if(e.substr(0,8)=='preload/')
            {
                this.loader_pre.add(e, this.imgModules[e]);
                this.arr_assetsPre.push(e);
            }
        })
    ...
    preloadComplete(loader, resource) {
        this.sp_logo = YR.Easy.CreateSprite('preload/logo.png', MyData.stageW / 2, MyData.stageH / 2 - 50, 0.5, 0.5, 1, resource);
